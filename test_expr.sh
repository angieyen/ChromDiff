...
a_label="expr.${a_option}"
b_label="expr.${b_option}"
pval_file="all_pvals/${model}/${metric}.${test_type}.${correction}.${property}.${a_label}.${b_label}.txt"
if [ ! -e $pval_file ]; then
	bsub -o jobs/expr.job -q compbio-week -P compbiofolk -R "rusage[mem=4]" "./perform_analysis.sh $property $a_label $b_label $test_type $correction $curr_label $heightcutoff $metadatafile $genefile $covariate_mat_file $map_covariates_file $expfile $state_annotations_file $generegions_label " 
fi;
